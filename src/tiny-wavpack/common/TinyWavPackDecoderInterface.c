 ...
 DecoderResult decode_wavpack_to_wav(
     ...
     uint32_t samples_to_decode = (max_samples == -1) ? numSamples :
                                 (max_samples > numSamples ? numSamples : max_samples);
     ...
     while (totalSamplesRead < samples_to_decode) {
         int samplesToRead = (samples_to_decode - totalSamplesRead < BUFFER_SIZE) ?
                             samples_to_decode - totalSamplesRead : BUFFER_SIZE;
